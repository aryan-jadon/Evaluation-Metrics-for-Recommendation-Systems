...
def mrr_at_k(
    ...
    """
    # Prepare the data and merge true/predicted ratings
    df_hit, df_hit_count, n_users = merge_ranking_true_pred(
        rating_true=rating_true,
        rating_pred=rating_pred,
        col_user=col_user,
    ...
    # If no hits, return 0.0
    if df_hit.shape[0] == 0:
        return 0.0

    # For each user, get the rank of the first relevant item
    df_hit_sorted = df_hit.sort_values([col_user, 'rank'])
    df_first_hit = df_hit_sorted.groupby(col_user).first().reset_index()

    # Compute the reciprocal ranks
    df_first_hit['reciprocal_rank'] = 1 / df_first_hit['rank']

    # Calculate the mean of the reciprocal ranks
    mrr = df_first_hit['reciprocal_rank'].mean()
...
def arhr_at_k(
    ...
    """Calculate Average Reciprocal Hit Rate (ARHR) at K.
    ...
    # Calculate reciprocal rank for hits
    df_hit['reciprocal_rank'] = 1 / df_hit['rank']

    # Calculate average of reciprocal ranks
    arhr = df_hit['reciprocal_rank'].mean()
...
def average_precision_at_k(
    ...
    """Calculate average precision at k for each user and average over all users.
    Args:
        rating_true (pandas.DataFrame): True DataFrame
        rating_pred (pandas.DataFrame): Predicted DataFrame
        k (int): number of top k items per user
    ...
    Returns:
        float: Average Precision at k (min=0, max=1)
    """
    df_hit, _, _ = merge_ranking_true_pred(
        rating_true=rating_true,
        rating_pred=rating_pred,
    ...
    df_hit['precision_at_k'] = df_hit.groupby(col_user).cumcount() / (df_hit['rank'] - 1)
    average_precision = df_hit.groupby(col_user)['precision_at_k'].mean().mean()
...
def average_recall_at_k(
    ...
    """Calculate average recall at k for each user and average over all users.
    Args:
        rating_true (pandas.DataFrame): True DataFrame
        rating_pred (pandas.DataFrame): Predicted DataFrame
        k (int): number of top k items per user
        ...
        relevancy_method (str): method for determining relevancy
    Returns:
        float: Average Recall at k (min=0, max=1)
    """
    df_hit, df_hit_count, _ = merge_ranking_true_pred(
        rating_true=rating_true,
        rating_pred=rating_pred,
        col_user=col_user,
        col_item=col_item,
    ...
    df_hit['recall_at_k'] = df_hit.groupby(col_user)['rank'].count() / df_hit_count['actual']
    average_recall = df_hit.groupby(col_user)['recall_at_k'].mean().mean()
...
metrics = {
    ...
    mrr_at_k.__name__: mrr_at_k,
    arhr_at_k.__name__: arhr_at_k,
    precision_at_k.__name__: precision_at_k,
    recall_at_k.__name__: recall_at_k,
    average_recall_at_k.__name__: average_recall_at_k,
    average_precision_at_k.__name__: average_precision_at_k,
