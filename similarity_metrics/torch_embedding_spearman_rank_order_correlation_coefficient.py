...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for i, batches in enumerate(dataloader):
            ...
            if i > 0 and i % 1000 == 0:
                ...
                logger.info(f"Epoch: {epoch}, Seq: {i:,}/{len(dataloader):,}, Loss: {running_loss:.4f}, "
                            f"Spearman: {spearman_corr:.4f}, Lr: {optimizer.param_groups[0]['lr']:.6f}")
                results.append([epoch, i, running_loss, spearman_corr])
                running_loss = 0
    ...
    results_df.to_csv('{}/model_metrics_spearman_rank_order_correlation_coefficient_similarity.csv'.format(MODEL_PATH),
                      index=False)
