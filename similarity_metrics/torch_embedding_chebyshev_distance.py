...
if __name__ == '__main__':
        ...
        for i, batches in enumerate(dataloader):
            ...
            if i > 0 and i % 1000 == 0:
                with torch.no_grad():
                    ...
                    chebyshev_distance_calculations = chebyshev_distance(product1_emb, product2_emb)
                    score = roc_auc_score(val_samp['edge'], chebyshev_distance_calculations.detach().cpu().numpy())
    ...
    results_df.to_csv('{}/model_metrics_chebyshev_distance_similarity.csv'.format(MODEL_PATH), index=False)
